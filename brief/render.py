...
def render_feishu(day: str, groups: list[tuple[str, list[dict]]], title: str) -> dict:
    """组装飞书 post 消息体。groups 是 [(栏目名, 条目列表), ...]。

    分栏是因为赛事内容对手柄的价值分天然低于大作动态，放在一起抢位次会永远排不上。
    """
    rows: list[list[dict]] = []
    all_items: list[dict] = []
    ...
    internal = [str(i) for i, it in enumerate(all_items, 1) if _is_internal(it)]
    if internal:
...
def render_markdown(day: str, groups: list[tuple[str, list[dict]]], title: str) -> str:
    ...
    for name, items in groups:
        ...
        for i, item in enumerate(items, 1):
            ...
            lines.append(f"- 命中：{'、'.join(item.get('hit_names', []))}")
            if item.get("verdict"):
    ...
    internal = [f"{i}. {it['title']}" for i, it in enumerate(all_items, 1) if _is_internal(it)]
    lines.append("## 值得转给同事")
    lines.append("")
